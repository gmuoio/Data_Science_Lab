...
potenza_u1 <- ggplot(u1, aes(x = timestamp, y = CONSUMO_ATTIVA_PRELEVATA))+
  geom_line(color = "#0795ff") + 
  xlab(label = "Tempo") +
  ylab(label = "Potenza media rilevata") +
  ylim(0,275)+
  ggtitle("U1") +
  theme(plot.title = element_text(hjust = 0.5))
potenza_u1

potenza_u6 <- ggplot(u6, aes(x = timestamp, y = CONSUMO_ATTIVA_PRELEVATA))+
...
grid.arrange(potenza_u1,potenza_u6)


# common_col <- colnames(u6)
# u1_u6_15min <- rbind(u1[,c("timestamp","CONSUMO_ATTIVA_PRELEVATA","EDIFICIO")],u6[,c("timestamp","CONSUMO_ATTIVA_PRELEVATA","EDIFICIO")])
# 
# grafico_potenza <-  ggplot(u1_u6_15min, aes(x = timestamp, y = CONSUMO_ATTIVA_PRELEVATA , colour=EDIFICIO)) +
#   geom_line()# + 
#   xlab(label = "Tempo") +
#   ylab(label = "Potenza media rilevata") +
#   ylim(500,4500)+
#   facet_wrap( ~ format(DATA, "%Y"), scales = "free_x")
# plot(grafico_potenza)
...
#boxplot affiancati
aux <- rbind(cbind(u1_daily[,colnames(u6_daily)], EDIFICIO='U1'), cbind(u6_daily, EDIFICIO='U6'))

ggplot(aux, aes(x=EDIFICIO, y=ENERGIA, fill=EDIFICIO)) + 
  geom_boxplot(outlier.colour="red", outlier.shape=16,
               outlier.size=1.5) +
  scale_fill_manual(values=c("#0795ff","#ffa500")) +
  ylab("Energia (kWh)") +
  xlab('Edificio') + 
...
#imputation sono le imputazioni, truth 
ggplot_na_imputations(x_with_na=u6_timeseries_NA,
                      x_with_truth=u6_seasplit_ma,
                      x_with_imputations = xts(u6_daily$ENERGIA,u6_daily$DATA),
                      title = element_blank(),
                      ...
                      color_imputations = "#0795ff",
                      color_truth = "#ffa500",
                      color_lines = "lightslategray",
                      shape_points = 16,
                      shape_imputations = 16,
                      shape_truth = 16,
                      size_points = 1.5,
                      size_imputations = 1.5,
                      size_truth = 1.5,
                      size_lines = 0.5,
                      label_imputations = "U1 Giugno",
                      label_truth = "U6 Giugno (imputati)")+
